...
class SchedulerService:
    ...
    async def _load_jobs(self):
        ...
        # Database backup job (daily at 3 AM)
        self.scheduler.add_job(
        ...
        # Airing schedule fetch job (every 6 hours)
        self.scheduler.add_job(
    ...
    async def cleanup_storage(self):
        ...
        try:
            ...
            # Clean old processed files (older than configured hours)
            cleanup_hours = int(await self.db_manager.get_config('cleanup_hours', str(Config.CLEANUP_HOURS)))
            cutoff_time = datetime.now() - timedelta(hours=cleanup_hours)
            
            # Clean download directory
            download_deleted = 0
            for file_path in Config.DOWNLOAD_PATH.glob("*"):
            ...
            processed_deleted = 0
            for file_path in Config.PROCESSED_PATH.glob("*"):
                if file_path.is_file():
                    mtime = datetime.fromtimestamp(file_path.stat().st_mtime)
                    if mtime < cutoff_time:
                        file_path.unlink()
            ...
            temp_deleted = 0
            for file_path in Config.TEMP_PATH.glob("*"):
                if file_path.is_file():
                    mtime = datetime.fromtimestamp(file_path.stat().st_mtime)
                    if mtime < cutoff_time:
                        file_path.unlink()
            ...
            # Clean old database tasks
            tasks_deleted = await self.db_manager.delete_old_tasks(days=30)
            
            self.logger.info(
                f"✅ Cleanup complete: "
                f"Downloads: {download_deleted}, "
                f"Processed: {processed_deleted}, "
                f"Temp: {temp_deleted}, "
    ...
    async def backup_database(self):
        ...
        try:
            ...
            backup_path = backup_dir / f"anime_bot_backup_{timestamp}.db"
    ...
    async def fetch_airing_schedule(self):
        ...
        try:
            ...
            # Send notification to admin channel if there are new episodes
            if airing:
    ...
    async def update_stats(self):
        ...
        try:
            ...
            self.logger.info(f"Stats: {stats.total_tasks} tasks, {stats.total_users} users")
    ...
    async def _notify_admins_pending_requests(self, requests: list):
        ...
        message = f"""
📋 <b>Pending Requests Notification</b>

There are <b>{len(requests)}</b> pending user requests waiting for approval.

<b>Recent requests:</b>
"""
        for req in requests[:5]:
            message += f"\n• #{req['request_id']} - {req['anime_title']} (User: {req['user_id']})"
        
        if len(requests) > 5:
            message += f"\n\n... and {len(requests) - 5} more"
        
        message += f"\n\nUse <code>/view_requests</code> to manage them.\n\n{Config.FOOTER}"
    ...
    async def _notify_new_episodes(self, airing_list: list):
        ...
        for anime in airing_list:
            airing_time = datetime.fromtimestamp(anime.get('airing_at', 0))
            if airing_time.date() == datetime.now().date():
                today_episodes.append(anime)
        
        if today_episodes:
            message = f"""
🎬 <b>New Episodes Airing Today!</b>

"""
            for anime in today_episodes:
                message += f"• <b>{anime['title']}</b> - Episode {anime['episode']}\n"
    ...
    async def _notify_stuck_tasks(self, task_ids: list):
        ...
        uploader = Uploader(self.db_manager)
        message = f"""
⚠️ <b>Stuck Tasks Detected</b>

The following tasks have been stuck for over 2 hours:
...
Free space: <b>{free_gb:.1f} GB</b>
Threshold: <b>{Config.CLEANUP_THRESHOLD_GB} GB</b>

Please run <code>/del_timer</code> to adjust cleanup schedule or free up space manually.
        """
